...
class TasksRepository:
    ...
    def create_task(self, message_id: str, task_type: TaskType, channel_id: int) -> TaskModel:
    ...
    def update_task_status(
        ...
        try:
            task_entity = self._session.query(TaskEntity).filter_by(message_id=message_id).first()
            if task_entity:
    ...
    def get_task_by_message_id(self, message_id: str) -> Optional[TaskModel]:
    ...
    def get_task_by_id(self, task_id: int) -> Optional[TaskModel]:
        """Get task by ID"""
        task_entity = self._session.query(TaskEntity).filter_by(id=task_id).first()
        return TaskModel.model_validate(task_entity) if task_entity else None

    def update_task_message_id(self, task_id: int, message_id: str) -> TaskModel:
    ...
    def get_tasks_by_channel(self, channel_id: int) -> List[TaskModel]:
        """Get all tasks for a specific channel"""
        task_entities = self._session.query(TaskEntity).filter_by(channel_id=channel_id).all()
        return [TaskModel.model_validate(entity) for entity in task_entities]
